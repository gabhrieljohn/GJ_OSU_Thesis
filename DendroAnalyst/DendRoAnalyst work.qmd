...
# Help Pages & Function Information
...
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------+
| [dm.detrend](http://127.0.0.1:42961/help/library/dendRoAnalyst/html/dm.detrend.html)                                                                                                                                                                                                                                                                                         | Detrend the dendrometer data                                                                                                               |
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------+
...
```{r}
#dendrometer data
#JUST 309 FOR NOW, NO FIXED RESETS
here(file="dscmet_422_all.RData") #calling saved R object from ecomatik.rmd
#checking bad flags - no bad ones found
unique_422_strings <- unique(
  unlist(
    sapply(
      dscmet_422_all, function(x) x[is.character(x)])))
#getting hourly averages
...
here(file="P230long.RData")
here(file="P230.RData")
...
## Prepping objects following protocols from DendRoAnalyst vignette

# Plugging in 309 to example code
...
read.den
 ...
 library(dendRoAnalyst)
relative_dry_growth<-clim.twd(df=d309_hourly, Clim=filtered_P230_rain_daily, dailyValue='max', showPlot=TRUE)
...
#keep getting errors with logical and character vectors mixing and making issues so let's patch the code locally with getAnywhere(daily.data)
daily.data.patched<-function (df, TreeNum) 
{
    if (TreeNum > ncol(df) - 1) {
        stop("The tree number is not valid for provided dataset.")
...
# Relative Dry Growth During Climate Adverse events
