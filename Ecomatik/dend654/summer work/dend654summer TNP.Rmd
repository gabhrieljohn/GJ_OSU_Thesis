...
```{r}
dend654tsedit <- read.csv("~/research/dend654/654tsedit.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend654tsedit)
```
...
```{r}
dend654<-dend654tsedit
datetime<-dend654$datetime
ID = '654'
ts = as.POSIXct(dend654['datetime'][,],format="%Y-%m-%d %H:%M:%S",tz="GMT") 
dend654_L0<-cbind(dend654["value"]+0,ts,series=ID)
```
...
sel<-dend654_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(min(dend654_L0$value,na.rm=T),
                max(dend654_L0$value,na.rm=T)),
         main=unique(dend654_L0$series))
...
?treenetproc::proc_L1 
dend654_L1<- proc_L1(data_L0 = dend654_L0, reso = 60 ) 
...
temp_data_654_L0 <- cbind(DSCMET421_mean56m,series=ID)
colnames(temp_data_654_L0) <-c('ts','value','series')

#Now we can time-align temperature data with proc_L1
temp_data_654_L1 <- proc_L1(data_L0 = temp_data_654_L0, reso = 60, 
                            date_format ="%Y-%m-%d %H:%M:%S", tz = "GMT") 
head(temp_data_654_L1)
```
...
dend654_L2 <- proc_dendro_L2(dendro_L1 = dend654_L1, temp_L1 = temp_data_654_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend654summer_full", tz="GMT") 
graphics.off()
```
...
```{r}
ggplot((subset(dend654_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend654_L2$yr <- substr(dend654_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend654_2023_growth <- dend654_L2[which(dend654_L2$yr == "2023"),] 
dend654_2022_growth <- dend654_L2[which(dend654_L2$yr == "2022"),]
dend654_2021_growth <- dend654_L2[which(dend654_L2$yr == "2021"),]
dend654_2020_growth <- dend654_L2[which(dend654_L2$yr == "2020"),]
dend654_2019_growth <- dend654_L2[which(dend654_L2$yr == "2019"),]
dend654_2018_growth <- dend654_L2[which(dend654_L2$yr == "2018"),]
...
dend654_2023_growth$gro_tot <- dend654_2023_growth$gro_yr+90040
dend654_2022_growth$gro_tot <- dend654_2022_growth$gro_yr+66629
dend654_2021_growth$gro_tot <- dend654_2021_growth$gro_yr+42349.5
dend654_2020_growth$gro_tot <- dend654_2020_growth$gro_yr+21734.5
dend654_2019_growth$gro_tot <- dend654_2019_growth$gro_yr+1901
dend654_2018_growth$gro_tot <- dend654_2018_growth$gro_yr

#recombine both years data frames into one. 
dend654_L3<-rbind(dend654_2018_growth,dend654_2019_growth,dend654_2020_growth,dend654_2021_growth,dend654_2022_growth, dend654_2023_growth)

#Now plot!
ggplot((subset(dend654_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
grow_seas_L2 <- grow_seas(dendro_L2 = dend654_L2,agg_yearly=TRUE,tol_seas = 0.1)
...
```{r}
ggplot((subset(dend654_L3, ts < as.POSIXct("2024-03-01 00:00") & ts > as.POSIXct("2023-03-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 654")
```
...
dend654_phase <- phase_stats(dend654_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend654_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend654_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend654_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend654_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend654full_cleaned.csv", dend654_L3)

#daily growth stats 
write.csv(file = "dendro_654full_phase_stats.csv", dend654_phase)
```
