...
```{r}
dend860tsedit <- read.csv("~/research/dend860/summer work/860tsedit.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend860tsedit)
```
...
temp_data_860_L0 <- cbind(DSCMET421_mean56m,series=ID)
...
dend860_2021_growth <- dend860_L2[which(dend860_L2$yr == "2021"),]
dend860_2020_growth <- dend860_L2[which(dend860_L2$yr == "2020"),]
dend860_2019_growth <- dend860_L2[which(dend860_L2$yr == "2019"),]

#"for 2023, add that years growth to 2022's maximum growth for 2022, make a copy of gro_yr"

dend860_2023_growth$gro_tot <- dend860_2023_growth$gro_yr+76789.33
dend860_2022_growth$gro_tot <- dend860_2022_growth$gro_yr+58246.33
dend860_2021_growth$gro_tot <- dend860_2021_growth$gro_yr+34859.33
dend860_2020_growth$gro_tot <- dend860_2020_growth$gro_yr+14000
dend860_2019_growth$gro_tot <- dend860_2019_growth$gro_yr

#recombine both years data frames into one. 
dend860_L3<-rbind(dend860_2019_growth,dend860_2020_growth,dend860_2021_growth,dend860_2022_growth, dend860_2023_growth)
...
```{r}
ggplot((subset(dend860_L3, ts < as.POSIXct("2024-03-01 00:00") & ts > as.POSIXct("2023-03-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 860")
```
...
```{r}
#for plotting 
write.csv(file = "dend860full_cleaned.csv", dend860_L3)

#daily growth stats 
write.csv(file = "dendro_860full_phase_stats.csv", dend860_phase)
```
