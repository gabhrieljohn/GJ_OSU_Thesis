...
```{r}
dend648ts <- read.csv("~/research/dend648ts.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend648ts)
```
...
temp_data_648_L0 <- cbind(mean4000C,series=ID)
...
dend648_L2 <- proc_dendro_L2(dendro_L1 = dend648_L1, temp_L1 = temp_data_648_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend648_full", tz="GMT") 
graphics.off()
```
...
```{r}
dend648_L2_monthly <- proc_dendro_L2(dendro_L1 = dend648_L1, temp_L1 = temp_data_648_L1, tol_out = 2, tol_jump = 3, frost_thr = 1, plot = TRUE, interpol = 3.5*60, plot_period = "monthly", plot_export = TRUE, plot_name = "dend648_monthly", tz="GMT") 
graphics.off()
```
...
dend648_2022_growth <- dend648_L2[which(dend648_L2$yr == "2022"),]
...
dend648_2022_growth$gro_tot <- dend648_2022_growth$gro_yr

#recombine both years data frames into one. 
dend648_L3<-rbind(dend648_2022_growth, dend648_2023_growth)
...
```{r}
ggplot((subset(dend648_L3, ts < as.POSIXct("2023-10-31 00:00") & ts > as.POSIXct("2023-04-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 648")
...
```{r}
#for plotting 
write.csv(file = "dend648_cleaned.csv", dend648_L3)

#daily growth stats 
write.csv(file = "dendro_648_phase_stats.csv", dend648_phase)
```
