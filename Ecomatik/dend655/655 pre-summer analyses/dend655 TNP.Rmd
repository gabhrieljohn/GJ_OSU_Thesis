...
```{r}
dend655ts <- read.csv("~/research/dend655ts.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend655ts)
```
...
temp_data_655_L0 <- cbind(mean4000C,series=ID)
...
dend655_L2 <- proc_dendro_L2(dendro_L1 = dend655_L1, temp_L1 = temp_data_655_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend655_full", tz="GMT") 
graphics.off()
```
...
```{r}
dend655_L2_monthly <- proc_dendro_L2(dendro_L1 = dend655_L1, temp_L1 = temp_data_655_L1, tol_out = 2, tol_jump = 3, frost_thr = 1, plot = TRUE, interpol = 3.5*60, plot_period = "monthly", plot_export = TRUE, plot_name = "dend655_monthly", tz="GMT") 
graphics.off()
```
...
dend655_2023_growth$gro_tot <- dend655_2023_growth$gro_yr+1000
dend655_2022_growth$gro_tot <- dend655_2022_growth$gro_yr

#recombine both years data frames into one. 
dend655_L3<-rbind(dend655_2022_growth, dend655_2023_growth)
...
```{r}
ggplot((subset(dend655_L3, ts < as.POSIXct("2023-10-31 00:00") & ts > as.POSIXct("2023-04-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 655")
...
```{r}
#for plotting 
write.csv(file = "dend655_cleaned.csv", dend655_L3)

#daily growth stats 
write.csv(file = "dendro_655_phase_stats.csv", dend655_phase)
```
