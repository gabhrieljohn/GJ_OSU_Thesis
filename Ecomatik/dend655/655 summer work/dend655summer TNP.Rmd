...
```{r}
dend655tsedit <- read.csv("~/research/dend655/655tsedit.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend655tsedit)
```
...
```{r}
dend655<-dend655tsedit
datetime<-dend655$datetime
ID = '655'
ts = as.POSIXct(dend655['datetime'][,],format="%Y-%m-%d %H:%M:%S",tz="GMT") 
dend655_L0<-cbind(dend655["value"]+0,ts,series=ID)
```
...
sel<-dend655_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(min(dend655_L0$value,na.rm=T),
                max(dend655_L0$value,na.rm=T)),
         main=unique(dend655_L0$series))
...
?treenetproc::proc_L1 
dend655_L1<- proc_L1(data_L0 = dend655_L0, reso = 60 ) 
...
temp_data_655_L0 <- cbind(DSCMET421_mean56m,series=ID)
colnames(temp_data_655_L0) <-c('ts','value','series')

#Now we can time-align temperature data with proc_L1
temp_data_655_L1 <- proc_L1(data_L0 = temp_data_655_L0, reso = 60, 
                            date_format ="%Y-%m-%d %H:%M:%S", tz = "GMT") 
head(temp_data_655_L1)
```
...
dend655_L2 <- proc_dendro_L2(dendro_L1 = dend655_L1, temp_L1 = temp_data_655_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend655summer_full", tz="GMT") 
graphics.off()
```
...
```{r}
ggplot((subset(dend655_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend655_L2$yr <- substr(dend655_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend655_2023_growth <- dend655_L2[which(dend655_L2$yr == "2023"),] 
dend655_2022_growth <- dend655_L2[which(dend655_L2$yr == "2022"),]
dend655_2021_growth <- dend655_L2[which(dend655_L2$yr == "2021"),]
dend655_2020_growth <- dend655_L2[which(dend655_L2$yr == "2020"),]
dend655_2019_growth <- dend655_L2[which(dend655_L2$yr == "2019"),]
dend655_2018_growth <- dend655_L2[which(dend655_L2$yr == "2018"),]
...
dend655_2023_growth$gro_tot <- dend655_2023_growth$gro_yr+126809
dend655_2022_growth$gro_tot <- dend655_2022_growth$gro_yr+94182
dend655_2021_growth$gro_tot <- dend655_2021_growth$gro_yr+57594
dend655_2020_growth$gro_tot <- dend655_2020_growth$gro_yr+25281
dend655_2019_growth$gro_tot <- dend655_2019_growth$gro_yr+3471
dend655_2018_growth$gro_tot <- dend655_2018_growth$gro_yr

#recombine both years data frames into one. 
dend655_L3<-rbind(dend655_2018_growth,dend655_2019_growth,dend655_2020_growth,dend655_2021_growth,dend655_2022_growth, dend655_2023_growth)

#Now plot!
ggplot((subset(dend655_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
grow_seas_L2 <- grow_seas(dendro_L2 = dend655_L2,agg_yearly=TRUE,tol_seas = 0.1)
...
```{r}
ggplot((subset(dend655_L3, ts < as.POSIXct("2024-03-01 00:00") & ts > as.POSIXct("2023-03-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 655")
```
...
dend655_phase <- phase_stats(dend655_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend655_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend655_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend655_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend655_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend655full_cleaned.csv", dend655_L3)

#daily growth stats 
write.csv(file = "dendro_655full_phase_stats.csv", dend655_phase)
```
