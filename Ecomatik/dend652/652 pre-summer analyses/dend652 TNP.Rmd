...
```{r}
dend652ts <- read.csv("~/research/dend652ts.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend652ts)
```
...
```{r}
dend652<-dend652ts
datetime<-dend652$datetime
ID = '652'
ts = as.POSIXct(dend652['datetime'][,],format="%Y-%m-%d %H:%M:%S",tz="GMT") 
dend652_L0<-cbind(dend652["value"]+0,ts,series=ID)
```
...
sel<-dend652_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(min(dend652_L0$value,na.rm=T),
                max(dend652_L0$value,na.rm=T)),
         main=unique(dend652_L0$series))
...
?treenetproc::proc_L1 
dend652_L1<- proc_L1(data_L0 = dend652_L0, reso = 60 ) 
...
temp_data_652_L0 <- cbind(mean4000C,series=ID)
colnames(temp_data_652_L0) <-c('ts','value','series')

#Now we can time-align temperature data with proc_L1
temp_data_652_L1 <- proc_L1(data_L0 = temp_data_652_L0, reso = 60, 
                            date_format ="%Y-%m-%d %H:%M:%S", tz = "GMT") 
head(temp_data_652_L1)
```
...
dend652_L2 <- proc_dendro_L2(dendro_L1 = dend652_L1, temp_L1 = temp_data_652_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend652_full", tz="GMT") 
graphics.off()
```
...
```{r}
dend652_L2_monthly <- proc_dendro_L2(dendro_L1 = dend652_L1, temp_L1 = temp_data_652_L1, tol_out = 2, tol_jump = 3, frost_thr = 1, plot = TRUE, interpol = 3.5*60, plot_period = "monthly", plot_export = TRUE, plot_name = "dend652_monthly", tz="GMT") 
graphics.off()
```
...
```{r}
ggplot((subset(dend652_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend652_L2$yr <- substr(dend652_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend652_2023_growth <- dend652_L2[which(dend652_L2$yr == "2023"),] 
dend652_2022_growth <- dend652_L2[which(dend652_L2$yr == "2022"),]

#"for 2023, add that years growth to 2022's maximum growth of 2214 µm #for 2022, make a copy of gro_yr"

dend652_2023_growth$gro_tot <- dend652_2023_growth$gro_yr
dend652_2022_growth$gro_tot <- dend652_2022_growth$gro_yr

#recombine both years data frames into one. 
dend652_L3<-rbind(dend652_2022_growth, dend652_2023_growth)

#Now plot!
ggplot((subset(dend652_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```

What if we just want ro look at the growing season?

```{r}
ggplot((subset(dend652_L3, ts < as.POSIXct("2023-10-31 00:00") & ts > as.POSIXct("2023-04-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 652")
...
dend652_phase <- phase_stats(dend652_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend652_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend652_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend652_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend652_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend652_cleaned.csv", dend652_L3)

#daily growth stats 
write.csv(file = "dendro_652_phase_stats.csv", dend652_phase)
```
