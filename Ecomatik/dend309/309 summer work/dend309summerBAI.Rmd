...
```{r}
dend309BAItsfull<-dend309tsfull|>mutate(area_mm2=dend_to_area_mm2(value))|>mutate(bai_mm2 = area_mm2 - lag(area_mm2))|>mutate(bai_cumu=cumsum(if_else(is.na(bai_mm2), 0, bai_mm2)))
```
...
```{r}
dend309BAI<-dend309BAItsfull
datetime<-dend309BAI$datetime
ID = '309'
ts = as.POSIXct(dend309BAI['datetime'][,],format="%Y-%m-%d %H:%M:%S",tz="GMT") 
dend309BAI_L0<-cbind(dend309BAI["bai_cumu"]+0,ts,series=ID)
dend309BAI_L0<-dend309BAI_L0|>
...
sel<-dend309BAI_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(0,max(dend309BAI_L0$value)),
         main="309 uncleaned BAI")
...
dend309BAI_L1<- proc_L1(data_L0 = dend309BAI_L0, reso = 60 ) 
...
temp_data_309BAI_L0 <- cbind(DSCMET421_mean56m,series=ID)
colnames(temp_data_309BAI_L0) <-c('ts','value','series')

#Now we can time-align temperature data with proc_L1
temp_data_309BAI_L1 <- proc_L1(data_L0 = temp_data_309BAI_L0, reso = 60, 
                            date_format ="%Y-%m-%d %H:%M:%S", tz = "GMT") 
head(temp_data_309BAI_L1)
```
...
dend309BAI_L2 <- proc_dendro_L2(dendro_L1 = dend309BAI_L1, temp_L1 = temp_data_309BAI_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend309BAIsummer_full", tz="GMT") 
graphics.off()
```
...
```{r}
ggplot((subset(dend309BAI_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend309BAI_L2$yr <- substr(dend309BAI_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend309BAI_2023_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2023"),] 
dend309BAI_2022_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2022"),]
dend309BAI_2021_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2021"),]
dend309BAI_2020_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2020"),]
dend309BAI_2019_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2019"),]
dend309BAI_2018_growth <- dend309BAI_L2[which(dend309BAI_L2$yr == "2018"),]
...
dend309BAI_2023_growth$gro_tot <- dend309BAI_2023_growth$gro_yr+208.121740
dend309BAI_2022_growth$gro_tot <- dend309BAI_2022_growth$gro_yr+171.625772
dend309BAI_2021_growth$gro_tot <- dend309BAI_2021_growth$gro_yr+107.204918
dend309BAI_2020_growth$gro_tot <- dend309BAI_2020_growth$gro_yr+62.504510
dend309BAI_2019_growth$gro_tot <- dend309BAI_2019_growth$gro_yr+8.734321
dend309BAI_2018_growth$gro_tot <- dend309BAI_2018_growth$gro_yr

#recombine both years data frames into one. 
dend309BAI_L3<-rbind(dend309BAI_2018_growth,dend309BAI_2019_growth,dend309BAI_2020_growth,dend309BAI_2021_growth,dend309BAI_2022_growth, dend309BAI_2023_growth)

#Now plot!
ggplot((subset(dend309BAI_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year")+ggtitle("Cumulative BAI ID 309")+ylab("Cumulative BAI (mm^2)")
```
...
dend309BAI_phase <- phase_stats(dend309BAI_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend309BAI_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend309BAI_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend309BAI_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend309BAI_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend309BAIfull_cleaned.csv", dend309BAI_L3)

#daily growth stats 
write.csv(file = "dendro_309BAIfull_phase_stats.csv", dend309BAI_phase)
```
...
```{r}
dend309BAIfull_cleaned <- read.csv("~/research/dend309/309 summer work/dend309BAIfull_cleaned.csv", stringsAsFactors=TRUE)
head(dend309BAIfull_cleaned)

dend309BAIfull_cleaned_L0<-dend309BAIfull_cleaned

#First grab years string
years <-left(dend309BAIfull_cleaned_L0[,"ts"],4)
#To plot, set number of plots with par()
par(mfrow=c(1,1))
#Then set margins 
par(mar = c(5, 5, 5, 5))
...
for(y in 1:length(unique(years))){
#Now we can select years
sel<-dend309BAIfull_cleaned_L0[which(years==unique(years)[y]),]
#But we need to handle first year first
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         col=rainbow(length(unique(years)))[y],pch=16,
         xlim=c(0,365),
         ylim=c(min(dend309BAIfull_cleaned_L0$value,na.rm=T),
                max(dend309BAIfull_cleaned_L0$value,na.rm=T)),
         main="309 BAI Cleaned")
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         col=rainbow(length(unique(years)))[y],pch=16,
         xlim=c(90,365),
         ylim=c(min(dend309BAIfull_cleaned_L0$gro_yr,na.rm=T),
                max(dend309BAIfull_cleaned_L0$gro_yr,na.rm=T)),
         main="309 BAI Cleaned")
    
    legend("topleft",
           as.character(unique(dend309BAIfull_cleaned_L0$yr)),
           col=rainbow(length(unique(dend309BAIfull_cleaned_L0$yr))),
           bty="n",pch=16)
# Now we can add other years
  }else{
    lines(difftime(as.POSIXct(sel$ts,format="%Y-%m-%d %H:%M:%S",tz="GMT"),
