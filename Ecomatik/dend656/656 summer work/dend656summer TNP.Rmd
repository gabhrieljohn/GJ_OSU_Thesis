...
```{r}
dend656tsedit <- read.csv("~/research/dend656/656tsedit.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend656tsedit)
```
...
```{r}
dend656<-dend656tsedit
datetime<-dend656$datetime
ID = '656'
ts = as.POSIXct(dend656['datetime'][,],format="%Y-%m-%d %H:%M:%S",tz="GMT") 
dend656_L0<-cbind(dend656["value"]+0,ts,series=ID)
```
...
sel<-dend656_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(min(dend656_L0$value,na.rm=T),
                max(dend656_L0$value,na.rm=T)),
         main=unique(dend656_L0$series))
...
?treenetproc::proc_L1 
dend656_L1<- proc_L1(data_L0 = dend656_L0, reso = 60 ) 
...
temp_data_656_L0 <- cbind(DSCMET421_mean56m,series=ID)
colnames(temp_data_656_L0) <-c('ts','value','series')

#Now we can time-align temperature data with proc_L1
temp_data_656_L1 <- proc_L1(data_L0 = temp_data_656_L0, reso = 60, 
                            date_format ="%Y-%m-%d %H:%M:%S", tz = "GMT") 
head(temp_data_656_L1)
```
...
dend656_L2 <- proc_dendro_L2(dendro_L1 = dend656_L1, temp_L1 = temp_data_656_L1, tol_out = 2, tol_jump = 3, plot = TRUE, frost_thr = 1, plot_period = "full", plot_export = TRUE, interpol = 3.5*60, frag_len = NULL, plot_name = "dend656summer_full", tz="GMT") 
graphics.off()
```
...
```{r}
ggplot((subset(dend656_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend656_L2$yr <- substr(dend656_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend656_2023_growth <- dend656_L2[which(dend656_L2$yr == "2023"),] 
dend656_2022_growth <- dend656_L2[which(dend656_L2$yr == "2022"),]
dend656_2021_growth <- dend656_L2[which(dend656_L2$yr == "2021"),]
dend656_2020_growth <- dend656_L2[which(dend656_L2$yr == "2020"),]
dend656_2019_growth <- dend656_L2[which(dend656_L2$yr == "2019"),]
dend656_2018_growth <- dend656_L2[which(dend656_L2$yr == "2018"),]
...
dend656_2023_growth$gro_tot <- dend656_2023_growth$gro_yr+96709.5
dend656_2022_growth$gro_tot <- dend656_2022_growth$gro_yr+77312.5
dend656_2021_growth$gro_tot <- dend656_2021_growth$gro_yr+49547
dend656_2020_growth$gro_tot <- dend656_2020_growth$gro_yr+24512
dend656_2019_growth$gro_tot <- dend656_2019_growth$gro_yr+3880
dend656_2018_growth$gro_tot <- dend656_2018_growth$gro_yr

#recombine both years data frames into one. 
dend656_L3<-rbind(dend656_2018_growth,dend656_2019_growth,dend656_2020_growth,dend656_2021_growth,dend656_2022_growth, dend656_2023_growth)

#Now plot!
ggplot((subset(dend656_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
grow_seas_L2 <- grow_seas(dendro_L2 = dend656_L2,agg_yearly=TRUE,tol_seas = 0.1)
...
```{r}
ggplot((subset(dend656_L3, ts < as.POSIXct("2024-03-01 00:00") & ts > as.POSIXct("2023-03-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 656")
```
...
dend656_phase <- phase_stats(dend656_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend656_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend656_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend656_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend656_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend656full_cleaned.csv", dend656_L3)

#daily growth stats 
write.csv(file = "dendro_656full_phase_stats.csv", dend656_phase)
```
