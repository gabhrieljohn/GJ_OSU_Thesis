...
```{r}
dend310tsfull <- read.csv("~/research/dend310/310tsfull.csv", na.strings="NaN", stringsAsFactors=TRUE)
head(dend310tsfull)
```
...
sel<-dend310_L0[which(years==unique(years)[y]),]
...
if(y==1){
  ...
  as.POSIXct(paste0(unique(years)[y],"-01-01 00:00:00"),
         ...
         xlim=c(0,365),
         ylim=c(min(dend310_L0$value,na.rm=T),
                max(dend310_L0$value,na.rm=T)),
         main=unique(dend310_L0$series))
...
```{r}
ggplot((subset(dend310_L2, frost == "FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_yr), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
dend310_L2$yr <- substr(dend310_L2$ts,1,4)
#Turn these columns into separate dataframes for each year
dend310_2023_growth <- dend310_L2[which(dend310_L2$yr == "2023"),] 
dend310_2022_growth <- dend310_L2[which(dend310_L2$yr == "2022"),]
dend310_2021_growth <- dend310_L2[which(dend310_L2$yr == "2021"),]
dend310_2020_growth <- dend310_L2[which(dend310_L2$yr == "2020"),]
dend310_2019_growth <- dend310_L2[which(dend310_L2$yr == "2019"),]
dend310_2018_growth <- dend310_L2[which(dend310_L2$yr == "2018"),]
...
dend310_2023_growth$gro_tot <- dend310_2023_growth$gro_yr+47695.57
dend310_2022_growth$gro_tot <- dend310_2022_growth$gro_yr+41950.57
dend310_2021_growth$gro_tot <- dend310_2021_growth$gro_yr+32958.57
dend310_2020_growth$gro_tot <- dend310_2020_growth$gro_yr+22486.07
dend310_2019_growth$gro_tot <- dend310_2019_growth$gro_yr+9823.5
dend310_2018_growth$gro_tot <- dend310_2018_growth$gro_yr

#recombine both years data frames into one. 
dend310_L3<-rbind(dend310_2018_growth,dend310_2019_growth,dend310_2020_growth,dend310_2021_growth,dend310_2022_growth, dend310_2023_growth)

#Now plot!
ggplot((subset(dend310_L3, frost =="FALSE")), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")
```
...
grow_seas_L2 <- grow_seas(dendro_L2 = dend310_L2,agg_yearly=TRUE,tol_seas = 0.1)
...
```{r}
ggplot((subset(dend310_L3, ts < as.POSIXct("2024-03-01 00:00") & ts > as.POSIXct("2023-03-01 00:00"))), aes(x=ts))+ geom_line(aes(y=value), color = "grey70")+ geom_line(aes(y=gro_tot), color = "seagreen")+ geom_line(aes(y=twd), color = "red")+xlab("Day of Year (2023)")+ggtitle("Cumulative Growing Season Growth ID 310")
```
...
dend310_phase <- phase_stats(dend310_L2, plot_phase = TRUE, plot_export = TRUE, agg_daily = TRUE, tz = 'GMT' )
...
ggplot(subset(dend310_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_amp, colour = "Shrinkage" ))+ geom_point(aes(y = exp_amp, colour ="Expansion"))+ ylab("Amplitude")

ggplot(subset(dend310_phase, doy > 100 & doy < 250), aes(x=doy))+ geom_point(aes(y = shrink_slope, colour = "Shrinkage" ))+ geom_point(aes(y = exp_slope, colour ="Expansion"))+ ylab("Slope")

ggplot(subset(dend310_phase, doy > 100 & doy < 250), aes(x=shrink_amp, y = exp_amp))+ geom_point()+ ylab(" Expansion amplitude")+ xlab(" Shrinkage amplitude")

ggplot(subset(dend310_phase, doy > 100 & doy < 250), aes(x=shrink_slope, y = exp_slope))+ geom_point()+ ylab(" Expansion slope")+ xlab(" Shrinkage slope")
```
...
```{r}
#for plotting 
write.csv(file = "dend310full_cleaned.csv", dend310_L3)

#daily growth stats 
write.csv(file = "dendro_310full_phase_stats.csv", dend310_phase)
```
